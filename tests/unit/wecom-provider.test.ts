...
import { WecomProvider, type WecomOAuthGateway, type WecomOAuthProfile } from '../../src/providers/wecom/wecom-provider.js';
...
/**
 * WecomProvider 回调逻辑单元测试。
 */
describe('WecomProvider', () => {
  it('应该在首次登录时创建用户与身份', async () => {
    const state = createWecomTestState();
    const provider = new WecomProvider(
      createWecomProviderConfig(),
      createFakeGateway({
        providerSubject: 'wecom-openid-001',
        displayName: '张三',
    ...
    assert.equal(state.createdUsers[0].email, 'zhangsan@example.com');
    assert.equal(state.createdIdentities.length, 1);
    assert.equal(state.createdIdentities[0].providerSubject, 'wecom-openid-001');
  ...
  it('应该在身份不存在时按邮箱绑定已有用户', async () => {
    ...
    const provider = new WecomProvider(
      createWecomProviderConfig(),
      createFakeGateway({
        providerSubject: 'wecom-openid-002',
        displayName: '邮箱用户',
        email: 'bind@example.com',
      }),
    );
  ...
  it('应该在邮箱和手机号命中不同用户时抛绑定冲突错误', async () => {
    ...
    const provider = new WecomProvider(
      createWecomProviderConfig(),
      createFakeGateway({
        providerSubject: 'wecom-openid-conflict',
        displayName: '冲突用户',
        email: 'conflict@example.com',
        phone: '13800001111',
      }),
    ...
    const provider = new WecomProvider(
      createWecomProviderConfig(),
      {
        resolveProfileByCode: async () => {
          gatewayCalled = true;
          return {
            providerSubject: 'wecom-openid-004',
            displayName: '无效状态用户',
          };
        },
      },
...
type MutableWecomTestState = WecomTestState & {
  useInvalidState?: boolean;
};

/**
 * 创建 WecomProvider 测试上下文。
 */
function createProviderContext(state: MutableWecomTestState): ProviderContext {
  const now = new Date();
 ...
 * 生成测试用企业微信网关。
 */
function createFakeGateway(profile: WecomOAuthProfile): WecomOAuthGateway {
  return {
...
/**
 * 创建测试用 WeCom Provider 配置。
 */
function createWecomProviderConfig() {
  return {
    type: 'wecom' as const,
    enabled: true,
 ...
 * 创建测试态对象。
 */
function createWecomTestState(input?: {
  users?: UserRecord[];
  identities?: IdentityRecord[];
}): MutableWecomTestState {
  return {
    users: [...(input?.users ?? [])],
